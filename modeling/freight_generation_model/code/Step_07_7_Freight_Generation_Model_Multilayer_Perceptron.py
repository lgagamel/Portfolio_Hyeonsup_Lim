...
# Output folder
# =============================================================================
os.chdir('output/step_7/Multilayer Perceptron')
...
# =============================================================================
def create_hidden_layer_sizes_options():
    hidden_layer_sizes_options = []
    for i in [3,4,5]:    
        for j in [1,2,3]:
            tmp_tuple = tuple([i for x in range(j)])
            hidden_layer_sizes_options = hidden_layer_sizes_options + [tmp_tuple]
    return hidden_layer_sizes_options

def run_GridSearch(X,y):
    hidden_layer_sizes_options = create_hidden_layer_sizes_options()
    tuned_parameters = [{'hidden_layer_sizes': hidden_layer_sizes_options,
                         'alpha': [0.00001,0.0001,0.001],
                         'activation': ['identity', 'logistic', 'relu']}]    
    grid = GridSearchCV(MLPRegressor(), tuned_parameters)
    grid.fit(X, y)
...
def run_MODEL_sub(X_train,X_test,y_train,y_test, target_measure_max, log_transform_ind, best_params_):
    model = MLPRegressor(hidden_layer_sizes = best_params_['hidden_layer_sizes'], activation = best_params_['activation'], alpha = best_params_['alpha'])
...
outputfile_all.write("model,measure,naics,N,log,attr,par,mae,rmse,r2,hidden_layer_sizes,activation,alpha\n")
outputfile_summary.write("model,measure,naics,N,log,attr,par,mae,rmse,r2,hidden_layer_sizes,activation,alpha\n") 

#target_measure = 'value'
model_approach = 'Multilayer Perceptron'
for target_measure in ['tons','value']:
    ...
    #                df[col] = df[col] + 10e-300
                ...
                outputfile_all.write(",".join([str(model_approach),
                                      ...
                                      '|'.join([str(x) for x in list(model_output['hidden_layer_sizes'])]),
                                      str(model_output['activation']),
                                      str(model_output['alpha']),
                                      ])+"\n")
                ...
                if model_output['mae_mean'] < min_mae:
                    ...
                    final_model_selection = {"log_transform":log_transform_ind,
                                   ...
                                   "model":model}
                #print("log_transform:",log_transform_ind,"tmp_attr_list:",tmp_attr_list, "mae_mean:",model_output['mae_mean'])
                #print(target_measure, naics_, tmp_attr_list, 'hidden_layer_sizes:',model_output['hidden_layer_sizes'])
            ...
            print(model_approach,
                  ...
                  "hidden_layer_sizes:",final_model_selection["model_output"]['hidden_layer_sizes'],
                  "activation:",final_model_selection["model_output"]['activation'],
                  "C:",final_model_selection["model_output"]['alpha'],)
            ...
            outputfile_summary.write(",".join([str(model_approach),
                                      ...
                                      '|'.join([str(x) for x in list(final_model_selection["model_output"]['hidden_layer_sizes'])]),
                                      str(final_model_selection["model_output"]['activation']),
                                      str(final_model_selection["model_output"]['alpha']),
                                      ])+"\n")
