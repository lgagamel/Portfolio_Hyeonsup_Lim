...
df_summary_pivot = pd.pivot_table(df_summary, values='mae', index=['measure', 'naics'],
                    columns=['model'], aggfunc=np.sum, fill_value=0)


df_summary_pivot.to_csv('output/step_8/1_Summary/summary_mae_by_model.csv')

df_summary_ = df_summary.groupby(['measure','naics'],as_index=False)['mae'].min()
df_summary_.columns = ['measure','naics','mae_min']


df_summary_OLS = df_summary.loc[df_summary["model"]=="OLS"].copy()
df_summary_OLS = df_summary_OLS.groupby(['measure','naics'],as_index=False)['mae'].min()
df_summary_OLS.columns = ['measure','naics','mae_OLS']
...
df_summary = df_summary.loc[df_summary['mae'] == df_summary['mae_min']]

ind = df_summary['model'] != "OLS"
ind = ind & (df_summary['mae_min']==df_summary['mae_OLS'])
