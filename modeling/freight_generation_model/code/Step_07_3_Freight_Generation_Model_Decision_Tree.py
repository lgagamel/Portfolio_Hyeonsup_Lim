...
# Output folder
# =============================================================================
os.chdir('output/step_7/Decision Tree')
...
def run_GridSearch(X,y):
    ...
    grid = GridSearchCV(DecisionTreeRegressor(), tuned_parameters)
    grid.fit(X, y)
...
def run_MODEL_sub(X_train,X_test,y_train,y_test, target_measure_max, log_transform_ind, best_params_):
    model = DecisionTreeRegressor(max_depth = best_params_['max_depth'], ccp_alpha = best_params_['ccp_alpha'])
...
outputfile_all.write("model,measure,naics,N,log,attr,par,mae,rmse,r2,max_depth,ccp_alpha\n")
outputfile_summary.write("model,measure,naics,N,log,attr,par,mae,rmse,r2,max_depth,ccp_alpha\n") 

#target_measure = 'value'
model_approach = 'Decision Tree'
for target_measure in ['tons','value']:
    ...
    #                df[col] = df[col] + 10e-300
            ...
            tree_text_representation = tree.export_text(final_model_selection["model"], feature_names=final_model_selection["attributes"])
            with open('by_naics/' + str(target_measure)+'_'+str(naics_)+"_tree.txt", "w") as fout:
                fout.write(tree_text_representation)
            
            fig = plt.figure(figsize=(25,20))
            _ = tree.plot_tree(final_model_selection["model"], 
                               feature_names=final_model_selection["attributes"],
                               filled=True)
            fig.savefig('by_naics/' + str(target_measure)+'_'+str(naics_)+"_tree.png")
