...
def run_GridSearch(X,y):
    tuned_parameters = [{'alpha': np.linspace(0, 0.02, 11)}]    
    grid = GridSearchCV(Lasso(fit_intercept=True,max_iter=10e5, positive=True), tuned_parameters)
    # grid = GridSearchCV(Lasso(fit_intercept=True,max_iter=10e5), tuned_parameters)
    grid.fit(X, y)
...
def run_MODEL_sub(X_train,X_test,y_train,y_test, target_measure_max, log_transform_ind, best_params_):
    model = Lasso(fit_intercept=True,max_iter=10e5, positive=True, alpha = best_params_['alpha'])
    # model = Lasso(fit_intercept=True,max_iter=10e5, alpha = best_params_['alpha'])
...
outputfile_all.write("model,measure,naics,N,log,attr,par,mae,rmse,r2,alpha\n")
outputfile_summary.write("model,measure,naics,N,log,attr,par,mae,rmse,r2,alpha\n") 

#target_measure = 'value'
model_approach = 'Lasso'
    ...
    #                df[col] = df[col] + 10e-300
                ...
                if model_output['mae_mean'] < min_mae:
                    ...
                    final_model_selection = {"log_transform":log_transform_ind,
                                   ...
                                   "model":model}
                #print("log_transform:",log_transform_ind,"tmp_attr_list:",tmp_attr_list, "mae_mean:",model_output['mae_mean'])
                #print(target_measure, naics_, tmp_attr_list, 'alpha:',model_output['alpha'])
            ...
            print(model_approach,
                  ...
                  "alpha:",final_model_selection["model_output"]['alpha'])
