...
# Output folder
# =============================================================================
os.chdir('output/step_7/Gaussian Process')
...
def create_kernel():
    ...
    for i in [0.5,1,1.5]:
        for j in [0.5,1,1.5]:
            kernel_ = DotProduct(sigma_0=i) + WhiteKernel(noise_level=j)
            kernel_list = kernel_list + [kernel_]
    return kernel_list

def run_GridSearch(X,y):
    kernel_list = create_kernel()
    tuned_parameters = [{'alpha': [1e-11,1e-10,1e-9],
                         'kernel': kernel_list}]
    grid = GridSearchCV(GaussianProcessRegressor(), tuned_parameters)
    grid.fit(X, y)
...
def run_MODEL_sub(X_train,X_test,y_train,y_test, target_measure_max, log_transform_ind, best_params_):
    model = GaussianProcessRegressor(kernel = best_params_['kernel'], alpha = best_params_['alpha'])
...
outputfile_all.write("model,measure,naics,N,log,attr,par,mae,rmse,r2,kernel,alpha\n")
outputfile_summary.write("model,measure,naics,N,log,attr,par,mae,rmse,r2,kernel,alpha\n") 

#target_measure = 'value'
model_approach = 'Gaussian Process'
