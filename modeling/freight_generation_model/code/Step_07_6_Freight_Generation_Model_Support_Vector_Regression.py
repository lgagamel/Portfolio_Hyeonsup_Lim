...
# Output folder
# =============================================================================
os.chdir('output/step_7/Support Vector Regression')
...
def run_GridSearch(X,y):
    tuned_parameters = [{'epsilon': np.linspace(0, 0.2, 11),
                          'C': np.linspace(0.1, 2.1, 11),
                          'kernel': ['linear', 'poly', 'rbf', 'sigmoid']}] 
    # tuned_parameters = [{'epsilon': [0,0.1],
    #                      'C': [0.1,1,2],
    #                      'kernel': ['linear', 'poly']}]
    grid = GridSearchCV(SVR(max_iter=1000), tuned_parameters)
    grid.fit(X, y)
...
def run_MODEL_sub(X_train,X_test,y_train,y_test, target_measure_max, log_transform_ind, best_params_):
    model = SVR(epsilon = best_params_['epsilon'], kernel = best_params_['kernel'], C = best_params_['C'], max_iter=1000)
...
outputfile_all.write("model,measure,naics,N,log,attr,par,mae,rmse,r2,epsilon,kernel,C\n")
outputfile_summary.write("model,measure,naics,N,log,attr,par,mae,rmse,r2,epsilon,kernel,C\n") 

#target_measure = 'value'
model_approach = 'Support Vector Regression'
for target_measure in ['tons','value']:
    ...
    #                df[col] = df[col] + 10e-300
                ...
                outputfile_all.write(",".join([str(model_approach),
                                      ...
                                      str(model_output['epsilon']),
                                      str(model_output['kernel']),
                                      str(model_output['C']),
                                      ])+"\n")
                ...
                if model_output['mae_mean'] < min_mae:
                    ...
                    final_model_selection = {"log_transform":log_transform_ind,
                                   ...
                                   "model":model}
                #print("log_transform:",log_transform_ind,"tmp_attr_list:",tmp_attr_list, "mae_mean:",model_output['mae_mean'])
                #print(target_measure, naics_, tmp_attr_list, 'epsilon:',model_output['epsilon'])
            ...
            print(model_approach,
                  ...
                  "epsilon:",final_model_selection["model_output"]['epsilon'],
                  "kernel:",final_model_selection["model_output"]['kernel'],
                  "C:",final_model_selection["model_output"]['C'],)
            ...
            outputfile_summary.write(",".join([str(model_approach),
                                      ...
                                      str(final_model_selection["model_output"]['epsilon']),
                                      str(final_model_selection["model_output"]['kernel']),
                                      str(final_model_selection["model_output"]['C']),
                                      ])+"\n")
